...
def load_cfg():
    try:
        with open(CONFIG_FILE) as f:
            ...
            d.setdefault("app_name", "PRODUCTION ORC1")
            return d
    except:
        return {"db_path":"","supervisor_pw":"1234","prod_ref":0,
                ...
                "modeles_horaires":[],"pilot_passwords":{},"app_name":"PRODUCTION ORC1"}
...
def api_lists():
    ...
    return jsonify({
        ...
        "copilotes": get_list("copilotes") or get_list("Co-Pilote") or get_list("Co-pilote") or get_list("Copilote") or get_list("Co-Pilotes") or get_list("Co-pilotes") or get_list("Copilotes") or [],
...
def api_config():
    ...
    return jsonify({
        ...
        "degrade_motifs": cfg.get("degrade_motifs",[]),
        "pers_pct_map": {str(k): round(v*100,1) for k,v in _pers_pct_map.items()},
        "app_name": cfg.get("app_name","PRODUCTION ORC1"),
    })
...
def api_settings():
    ...
    if "app_name" in data and str(data["app_name"]).strip():
        cfg["app_name"] = str(data["app_name"]).strip()
...
<div id="v-login" class="view on">
  <div class="login-card">
    <div style="display:flex;flex-direction:column;align-items:center;justify-content:center;gap:8px;margin-bottom:16px">
      ...
      <div id="login-app-name" class="lc-h1" style="margin-bottom:0;white-space:nowrap;font-size:calc(22px*var(--zf,1));letter-spacing:.5px">PRODUCTION ORC1</div>
    </div>
    <div class="lf">
      <label>Pilote</label>
  ...
  <div id="v-settings" class="view" style="flex-direction:column;overflow-y:auto">
    ...
    <div id="v-settings-content">
      ...
      <div class="ss">
        ...
        <div style="font-size:calc(11px*var(--zf,1));color:var(--gray);margin-bottom:12px">Recharge toutes les données depuis Excel, ou regénère le fichier <code id="dash-filename-hint">Rapport ORC1.html</code> dans le dossier du logiciel.</div>
      ...
      <div class="ss">
        ...
        <div style="font-size:calc(11px*var(--zf,1));color:var(--gray);margin-bottom:8px">Nom affiché sur l'écran de connexion, le bandeau, le titre de la fenêtre et le fichier dashboard généré.</div>
        <div class="flex" style="gap:8px;align-items:center;flex-wrap:wrap">
          <input id="cfg-app-name" placeholder="ex: PRODUCTION ORC1" style="flex:1;min-width:200px;padding:7px 10px;border:1.5px solid var(--border);border-radius:5px;font-size:calc(13px*var(--zf,1));font-weight:600">
          <button class="btn btn-ok" onclick="saveAppName()">💾 Enregistrer</button>
        </div>
      </div>
      <div class="ss">
...
<div class="overlay" id="m-nett-type" style="display:none;position:fixed;inset:0;background:rgba(0,0,0,.72);z-index:700;align-items:center;justify-content:center;backdrop-filter:blur(8px);-webkit-backdrop-filter:blur(8px)" onclick="if(event.target===this)closeM('m-nett-type')">
  <div class="mbox" style="width:380px;max-width:90vw;padding:24px" onclick="event.stopPropagation()">
    ...
    <div style="display:flex;flex-direction:column;gap:12px">
      <button class="btn" id="nett-btn-court" style="text-align:center;padding:14px 18px;border-radius:14px;border:2px solid #f97316;background:linear-gradient(145deg,#fff7ed,#fed7aa);font-size:calc(14px*var(--zf,1));font-weight:800;color:#9a3412;transition:all .15s;box-shadow:0 5px 0 #c2410c,0 7px 18px rgba(249,115,22,.22),inset 0 1px 0 rgba(255,255,255,.9)"
        ...
        onmouseup="this.style.transform='translateY(-2px)';this.style.boxShadow='0 7px 0 #c2410c,0 10px 24px rgba(249,115,22,.32),inset 0 1px 0 rgba(255,255,255,.9)'"
      ...
      <button class="btn" id="nett-btn-long" style="text-align:center;padding:14px 18px;border-radius:14px;border:2px solid #ea580c;background:linear-gradient(145deg,#fff7ed,#fdba74);font-size:calc(14px*var(--zf,1));font-weight:800;color:#9a3412;transition:all .15s;box-shadow:0 5px 0 #b45309,0 7px 18px rgba(234,88,12,.22),inset 0 1px 0 rgba(255,255,255,.9)"
        ...
        onmouseup="this.style.transform='translateY(-2px)';this.style.boxShadow='0 7px 0 #b45309,0 10px 24px rgba(234,88,12,.32),inset 0 1px 0 rgba(255,255,255,.9)'"
      ...
      <button class="btn" id="nett-btn-grand" style="text-align:center;padding:14px 18px;border-radius:14px;border:2px solid #c2410c;background:linear-gradient(145deg,#fff7ed,#fb923c);font-size:calc(14px*var(--zf,1));font-weight:800;color:#7c2d12;transition:all .15s;box-shadow:0 5px 0 #92400e,0 7px 18px rgba(194,65,12,.25),inset 0 1px 0 rgba(255,255,255,.85)"
        ...
        onmouseup="this.style.transform='translateY(-2px)';this.style.boxShadow='0 7px 0 #92400e,0 10px 24px rgba(194,65,12,.35),inset 0 1px 0 rgba(255,255,255,.85)'"
...
<div id="m-degrade" class="overlay" style="display:none;position:fixed;inset:0;background:rgba(0,0,0,.72);z-index:5100;align-items:center;justify-content:center;backdrop-filter:blur(8px);-webkit-backdrop-filter:blur(8px)" onclick="if(event.target===this)closeM('m-degrade')">
  <div class="mbox" style="max-width:400px;width:90%;padding:24px" onclick="event.stopPropagation()">
...
async function loadModelsForLogin() {
  ...
  _applyAppName(d.app_name||'PRODUCTION ORC1');
...
function _renderAndOpenOfDetail(r, ofEvts) {
  ...
  const tc = r.trs>=70?'#16a34a':r.trs>=50?'#f59e0b':r.trs>=0?'#dc2626':'#94a3b8';
...
function _kpiTrsColor(t){return t>=70?'#16a34a':t>=50?'#f59e0b':t>=0?'#dc2626':'#94a3b8';}
...
async function calcPeriodReport(autoLoad){
  ...
  const trsCol=d.trs_periode>=70?'#16a34a':d.trs_periode>=50?'#f59e0b':d.trs_periode>=0?'#dc2626':'#94a3b8';
  ...
  if(d.sessions_detail&&d.sessions_detail.length>0){
    ...
    sd.forEach((s,i)=>{
      ...
      if(s.trs>=0){const _ty=Math.max(by-3,12);const _lb=s.trs.toFixed(0)+'%';const _lw=_lb.length*7+6;svgLabels+=`<rect x="${cx-_lw/2}" y="${_ty-11}" width="${_lw}" height="14" fill="white" rx="2" opacity=".9"/><text x="${cx}" y="${_ty}" text-anchor="middle" font-size="11" font-weight="700" fill="${col}">${_lb}</text>`;}
    ...
    const trsTargetLine=`<line x1="0" y1="${_trs70Y}" x2="${svgW}" y2="${_trs70Y}" stroke="#16a34a" stroke-width="2" stroke-dasharray="6,3"/><text x="${svgW-2}" y="${_trs70Y-3}" text-anchor="end" font-size="10" fill="#16a34a" font-weight="700">Cible 70%</text>`;
  ...
  if(d.sessions_detail&&d.sessions_detail.length>0){
    ...
    const _CAD_CIBLE=720;
    const maxCad=Math.max(...sd2.map(s=>s.cadence_h||0),_CAD_CIBLE,1);
    ...
    sd2.forEach((s,i)=>{
      ...
      if(v>0){const _ty2=Math.max(by-3,12);const _lb2=String(v);const _lw2=_lb2.length*8+6;svgLabels2+=`<rect x="${cx2-_lw2/2}" y="${_ty2-12}" width="${_lw2}" height="15" fill="white" rx="2" opacity=".9"/><text x="${cx2}" y="${_ty2}" text-anchor="middle" font-size="13" font-weight="700" fill="${col}">${_lb2}</text>`;}
      const dp=s.date.split('/');
      if(_vertB){
    ...
    const tLine=`<line x1="0" y1="${tY}" x2="${svgW2}" y2="${tY}" stroke="#1d4ed8" stroke-width="2" stroke-dasharray="6,3"/><text x="${svgW2-2}" y="${tY-3}" text-anchor="end" font-size="10" fill="#1d4ed8" font-weight="700">Cible ${_CAD_CIBLE}</text>`;
  ...
  if(allOfs.length){
    const ofRows=allOfs.map((r,i)=>{
      ...
      <td style="padding:4px 6px;color:#0369a1">${esc(r.type_prod||'—')}</td>
  ...
  resultEl.innerHTML=`
    <div style="display:flex;gap:10px;height:100%;min-height:0">
      ...
      <div style="flex:0 0 260px;display:flex;flex-direction:column;gap:4px;overflow-y:auto">
        <div class="fp-card" style="padding:10px 14px"><div style="font-size:calc(19px*var(--zf,1));font-weight:800;color:${_colPcsRj}">${Math.round(d.tot_pcs||0)} <span style="font-weight:600;color:var(--gray)">Pièces</span> <span style="font-size:calc(14px*var(--zf,1));font-weight:600;color:#94a3b8">(Obj ${(d.objectif_pcs||0)>0?Math.round(d.objectif_pcs):'—'})</span></div></div>
        <div class="fp-card" style="padding:10px 14px"><div style="font-size:calc(19px*var(--zf,1));font-weight:800;color:${_colEquivRj}">${Math.round(d.tot_equiv||0)} <span style="font-weight:600;color:var(--gray)">Equiv</span> <span style="font-size:calc(14px*var(--zf,1));font-weight:600;color:#94a3b8">(Obj ${(d.objectif_equiv||0)>0?Math.round(d.objectif_equiv):'—'})</span></div></div>
        <div class="fp-card" style="padding:10px 14px"><div style="font-size:calc(19px*var(--zf,1));font-weight:800;color:${_colCadRj}">Cad/h : ${d.cadence_h||0} <span style="font-size:calc(14px*var(--zf,1));font-weight:600;color:#94a3b8">(ref : ${Math.round((d.cadence_ref_pcs_min||0)*100)/100}/min)</span></div></div>
      ...
      <!-- Droite : TRS+pie en haut, graphiques+pareto, tables -->
      <div style="flex:1;min-width:0;display:flex;flex-direction:column;gap:6px;overflow-y:auto">
        <!-- TRS % + camembert au-dessus des graphiques -->
        <div style="display:flex;gap:10px;align-items:center;background:var(--card-bg,#fff);border:1px solid var(--border);border-radius:8px;padding:10px 14px;flex-shrink:0">
        ...
        <!-- Graphiques (504px) + Pareto -->
        <div style="display:flex;gap:6px;flex-shrink:0;align-items:flex-start">
          <div style="flex:0 0 504px;display:flex;flex-direction:column;gap:6px">${chartTrsHtml}${chartCadHtml}</div>
...
async function captureRapportJour(){
  const v=window._rjVars;
  ...
  const _titleL1='Rapport ligne Oreillers Cardés 1'+(_date1!==_date2?' du '+_date1+' au '+_date2:' du '+_date1);
  // Titre ligne 2 : liste des postes avec dates
  const _posteNames={matin:'Matin',midi:'Midi',nuit:'Nuit',am:'Matin',pm:'Midi',night:'Nuit'};
  const _posteLbls=(d.sessions_detail||[]).map(s=>{
    const _pn=_posteNames[(s.poste||'').toLowerCase()]||esc(s.poste||'Poste');
    return `${_pn} ${esc(s.date||'')}${s.pilot?' ('+esc(s.pilot)+')':''}`;
  });
  const _titleL2=_posteLbls.length?_posteLbls.join(' · '):'';
  // Lignes stats (fonts +20%)
  const _statsRows=[['Temps d\'ouverture',Math.round(d.ouverture_min||0)+' min','#64748b'],['Temps utile',Math.round(d.temps_utile_min||0)+' min','#64748b'],['Tps de fonctionnement',Math.round(d.temps_fonctionnement_min||0)+' min',_colFonctRj],['Temps d\'arrêt',Math.round(d.net_stop_min||0)+' min',_colArretRj],['Temps d\'arrêt imprévu',Math.max(0,Math.round((d.net_stop_min||0)-(d.arret_prevu_min||0)))+' min',_colImpRj],['Temps arrêts prévus',Math.round(d.arret_prevu_min||0)+' min','#64748b'],['Dépass. arrêts prévu',(d.depassement_min||0)>0?Math.round(d.depassement_min)+' min':'✓ OK',(d.depassement_min||0)>0?'#dc2626':'#16a34a'],['Temps dégradé',Math.round(d.tot_degrade_min||0)+' min',_colDegRj],['Perte cadence',pertRaw>0?Math.round(pertRaw)+' min de perte':pertRaw<0?Math.abs(Math.round(pertRaw))+' min de gain':'0 min',_colPerteRj],['Postes',d.nb_sessions,'#64748b'],['Nombre d\'OF',d.nb_of,'#64748b'],['Nombre de chgt fibre',d.nb_fibre_chg||0,'#64748b']].map(([l,val,c])=>`<div style="display:flex;justify-content:space-between;align-items:center;padding:6px 10px;background:#fff;border:1px solid #e2e8f0;border-radius:4px;margin-bottom:3px"><span style="font-size:14px;color:#64748b;font-family:Arial,sans-serif">${esc(String(l))}</span><span style="font-size:15px;font-weight:700;color:${c};font-family:Arial,sans-serif">${esc(String(val))}</span></div>`).join('');
  // Layout capture : col gauche (stats+TRS+pie) | col droite (graphiques+pareto)
  const _capHtml=`<div style="display:flex;gap:14px;align-items:flex-start;font-family:Arial,sans-serif;zoom:1.25">
    <!-- Colonne gauche : stats + TRS + pie -->
    <div style="width:310px;flex-shrink:0;display:flex;flex-direction:column;gap:6px">
      <div style="background:#fff;border:1px solid #e2e8f0;border-radius:8px;padding:12px 16px"><div style="font-size:23px;font-weight:800;color:${_colPcsRj}">${Math.round(d.tot_pcs||0)} <span style="font-weight:600;color:#374151">Pièces</span> <span style="font-size:17px;font-weight:600;color:#94a3b8">(Obj ${(d.objectif_pcs||0)>0?Math.round(d.objectif_pcs):'—'})</span></div></div>
      <div style="background:#fff;border:1px solid #e2e8f0;border-radius:8px;padding:12px 16px"><div style="font-size:23px;font-weight:800;color:${_colEquivRj}">${Math.round(d.tot_equiv||0)} <span style="font-weight:600;color:#374151">Equiv</span> <span style="font-size:17px;font-weight:600;color:#94a3b8">(Obj ${(d.objectif_equiv||0)>0?Math.round(d.objectif_equiv):'—'})</span></div></div>
      <div style="background:#fff;border:1px solid #e2e8f0;border-radius:8px;padding:12px 16px"><div style="font-size:20px;font-weight:800;color:${_colCadRj}">Cad/h : ${d.cadence_h||0} <span style="font-size:15px;font-weight:600;color:#94a3b8">(ref : ${Math.round((d.cadence_ref_pcs_min||0)*100)/100}/min)</span></div></div>
      ${_statsRows}
      <!-- TRS période + camembert dans la colonne gauche -->
      <div style="background:#fff;border:1px solid #e2e8f0;border-radius:8px;padding:12px 16px;text-align:center;margin-top:4px">
        <div style="font-size:46px;font-weight:900;color:${trsCol};line-height:1">${d.trs_periode>=0?d.trs_periode.toFixed(1)+'%':'—'}</div>
        <div style="font-size:13px;color:#64748b;font-weight:600;margin-top:5px">TRS période</div>
        <div style="font-size:12px;color:#94a3b8;margin-top:2px">${d.nb_jours}j · ${d.nb_sessions} postes · ${d.nb_of} OF</div>
      </div>
      ${pieSmall?`<div style="background:#fff;border:1px solid #e2e8f0;border-radius:8px;padding:10px 12px;display:flex;justify-content:center">${pieSmall}</div>`:''}
    </div>
  ...
  _wrapper.style.cssText='position:fixed;left:-9999px;top:0;background:#f8fafc;padding:0;font-family:Arial,sans-serif';
  _wrapper.innerHTML=`
    <div style="background:#1e3a5f;color:#fff;padding:14px 22px;font-family:Arial,sans-serif">
      <div style="font-size:22px;font-weight:700;letter-spacing:.3px">${esc(_titleL1)}</div>
      ${_titleL2?`<div style="font-size:14px;font-weight:400;opacity:.85;margin-top:4px">${esc(_titleL2)}</div>`:''}
    </div>
    <div style="padding:14px">${_capHtml}</div>`;
...
async function loadCfg(){
  ...
  // App name
  const appNm=d.app_name||'PRODUCTION ORC1';
  const anEl=document.getElementById('cfg-app-name');
  if(anEl) anEl.value=appNm;
  _applyAppName(appNm);
...
let _appName='PRODUCTION ORC1';
function _applyAppName(name){
  _appName=name||'PRODUCTION ORC1';
  const ln=document.getElementById('login-app-name');
  if(ln)ln.textContent=_appName;
  const hd=document.getElementById('hdr-app-name');
  if(hd)hd.textContent=_appName;
  document.title=_appName;
  // Update dashboard filename hint
  const _safe=_appName.replace(/[\\/:*?"<>|]/g,'_');
  const fh=document.getElementById('dash-filename-hint');
  if(fh)fh.textContent='Rapport '+_safe+'.html';
}
async function saveAppName(){
  const v=(document.getElementById('cfg-app-name')?.value||'').trim();
  if(!v){toast('Nom requis','err');return;}
  const r=await fetch('/api/settings',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({pw:_adminPw,app_name:v})});
  const d=r?await r.json():{};
  if(d&&d.ok){_applyAppName(v);toast('Nom enregistré','ok');}else toast(d?.error||'Erreur','err');
}
async function generateDashboard(){
  ...
  const _safe=_appName.replace(/[\\/:*?"<>|]/g,'_');
  if(d.ok){
    if(st)st.textContent='✓ Rapport '+_safe+'.html généré à '+new Date().toLocaleTimeString('fr-FR',{hour:'2-digit',minute:'2-digit'});
    toast('Rapport '+_safe+' généré !','ok');
  }else{
...
def generate_dashboard_html():
    ...
    try:
        ...
        inject = (
            ...
            '  var d=window.DASH||{};\n'
            '  if(url.indexOf("/api/state")!==-1)return _DASH_STATE;\n'
            ...
            '    try{var params=new URL(url,location.href).searchParams;\n'
            '    var key=params.get("date")+"||"+params.get("pilot")+"||"+params.get("poste");\n'
            '    return d.session_reports[key]||{};}catch(e){return {};}\n'
            '  }\n'
            '  if(url.indexOf("/api/period_report")!==-1){\n'
            '    try{var pu=new URL(url,location.href).searchParams;\n'
            '    var ms=pu.get("max_sessions"),sk=pu.get("skip_current");\n'
            '    if(ms==="3"&&sk==="1")return d.period_last3||{};\n'
            '    var df=pu.get("date_from")||"",dt2=pu.get("date_to")||"";\n'
            '    if(!df)return d.period_last3||{};\n'
            '    var daysMs=(new Date(dt2)-new Date(df))/86400000;\n'
            ...
            '    });\n'
            '    document.title=' + _json.dumps(cfg.get("app_name","ORC1")) + ';\n'
        ...
        import re as _re
        _safe_name = _re.sub(r'[\\/:*?"<>|]', '_', _an)
        output_path = os.path.join(base_dir, f'Rapport {_safe_name}.html')
