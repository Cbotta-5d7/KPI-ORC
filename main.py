  ...
  if(d.sessions_detail&&d.sessions_detail.length>0){
    ...
    sd.forEach((s,i)=>{
      ...
      if(_vertA){
        ...
        svgL+=`<text transform="rotate(-90,${cx},${yA})" x="${cx}" y="${yA}" text-anchor="end" font-size="13" font-weight="600" fill="#374151">${_lbl}</text>`;
      }else{
        svgL+=`<text x="${cx}" y="${padT+gH+16}" text-anchor="middle" font-size="14" font-weight="600" fill="#374151">${esc((dp[0]||'')+'/'+(dp[1]||''))}</text>`;
        svgL+=`<text x="${cx}" y="${padT+gH+30}" text-anchor="middle" font-size="13" fill="#6366f1">${esc((s.pilot||'').slice(0,9))}</text>`;
        svgL+=`<text x="${cx}" y="${padT+gH+44}" text-anchor="middle" font-size="13" fill="#94a3b8">${esc((s.poste||'').slice(0,9))}</text>`;
      }
    });
    const yBase=padT+gH;
  ...
  if(d.sessions_detail&&d.sessions_detail.length>0){
    ...
    sd2.forEach((s,i)=>{
      ...
      if(_vertB){
        ...
        svgL2+=`<text transform="rotate(-90,${cx2},${yB})" x="${cx2}" y="${yB}" text-anchor="end" font-size="13" font-weight="600" fill="#374151">${_lbl2}</text>`;
      }else{
        svgL2+=`<text x="${cx2}" y="${padT2+gH2+16}" text-anchor="middle" font-size="14" font-weight="600" fill="#374151">${esc((dp[0]||'')+'/'+(dp[1]||''))}</text>`;
        svgL2+=`<text x="${cx2}" y="${padT2+gH2+30}" text-anchor="middle" font-size="13" fill="#6366f1">${esc((s.pilot||'').slice(0,9))}</text>`;
        svgL2+=`<text x="${cx2}" y="${padT2+gH2+44}" text-anchor="middle" font-size="13" fill="#94a3b8">${esc((s.poste||'').slice(0,9))}</text>`;
      }
    });
    ...
    const rows3=d.stop_pareto.map(e=>{
      ...
      return `<div style="margin-bottom:5px">
        <div style="display:flex;align-items:center;gap:4px;margin-bottom:2px">
          ...
          <span style="font-size:calc(10px*var(--zf,1));color:#374151;word-break:break-word;line-height:1.2"><b>${e.count}×</b> : ${esc(e.type)}</span>
        </div>
        <div style="display:flex;align-items:center;gap:4px">
...
async function captureRapportJour(){
  ...
  const _capHtml=`<div style="display:flex;gap:14px;align-items:flex-start;font-family:Arial,sans-serif;zoom:0.94">
    ...
    <div style="width:272px;flex-shrink:0;display:flex;flex-direction:column;gap:5px">
    ...
    <div style="width:262px;flex-shrink:0;display:flex;flex-direction:column;gap:3px">
      ${pieSmall?`<div style="background:#fff;border:1px solid #e2e8f0;border-radius:8px;padding:8px;display:flex;align-items:center;justify-content:center;margin-bottom:4px">${pieSmall}</div>`:''}
      ${_statsRows}
    </div>
  ...
  // Bandeau bleu : titre + TRS% en haut à droite
  const _trsBadgeColor=trsCol||'#fff';
  _wrapper.innerHTML=`
    <div style="background:#1e3a5f;color:#fff;padding:14px 22px;font-family:Arial,sans-serif;display:flex;align-items:center;justify-content:space-between;gap:20px">
      <div>
      ...
      <div style="text-align:center;flex-shrink:0">
        <div style="font-size:44px;font-weight:900;color:${_trsBadgeColor};line-height:1">${d.trs_periode>=0?d.trs_periode.toFixed(1)+'%':'—'}</div>
        <div style="font-size:12px;color:rgba(255,255,255,.75);font-weight:600;margin-top:3px">TRS période</div>
        <div style="font-size:11px;color:rgba(255,255,255,.55);margin-top:1px">${d.nb_jours}j · ${d.nb_sessions} postes</div>
      </div>
    </div>
...
async function loadSessionReport(date,pilot,poste,itemId){
  ...
  const _stopCounts={};
  (d.evt_rows||[]).filter(r=>!r.is_degrade&&!_isPlannedSr(r.type||'')).forEach(r=>{const k=r.type||'Inconnu';_stopCounts[k]=(_stopCounts[k]||0)+1;});
  const paretoHtml=stopArr.length?stopArr.map(([k,v])=>`
    <div style="margin-bottom:7px">
      <div style="font-size:calc(10px*var(--zf,1));color:var(--text);overflow:hidden;text-overflow:ellipsis;margin-bottom:2px"><b>${_stopCounts[k]||1}×</b> : ${esc(k)}</div>
  ...
  const _degCounts={};
  (d.evt_rows||[]).filter(r=>r.is_degrade).forEach(r=>{const k=r.type||'Mode dégradé';_degCounts[k]=(_degCounts[k]||0)+1;});
  const paretoDegHtml=degArrSr.length?degArrSr.map(([k,v])=>`
    <div style="margin-bottom:7px">
      <div style="font-size:calc(10px*var(--zf,1));color:var(--text);overflow:hidden;text-overflow:ellipsis;margin-bottom:2px"><b>${_degCounts[k]||1}×</b> : ${esc(k)}</div>
      <div style="display:flex;align-items:center;gap:5px">
        <div style="flex:1;background:#f1f5f9;border-radius:4px;height:12px;overflow:hidden">
  ...
  if(leftList&&leftKpi&&leftExe){
    ...
    leftKpi.innerHTML=`
      ...
      <div style="padding:6px 8px;display:flex;flex-direction:column;gap:5px">
        ...
        <div class="fp-card" style="padding:5px 6px"><div class="fp-big" style="font-size:calc(10px*var(--zf,1));font-weight:800;color:${_colCadRp}">${d.is_live?'Cad/h : —':`Cad/h : ${cadenceH}`} <span style="font-size:calc(9px*var(--zf,1));font-weight:600;color:#94a3b8">(ref : ${Math.round(cadenceRefPcsMin*10)/10}/min)</span></div></div>
...
def generate_dashboard_html():
    ...
    try:
        ...
        inject = (
            ...
            '        var pFill={fgColor:{rgb:"EFF6FF"}};var aFill={fgColor:{rgb:"FFFBEB"}};\n'
            '        for(var c=0;c<hdrs.length;c++){var ca=XLSX.utils.encode_cell({r:0,c:c});if(ws[ca])ws[ca].s=hdrS;}\n'
            '        for(var ri=0;ri<rows.length;ri++){\n'
            '          var ip=rows[ri].is_prod;\n'
            '          for(var ci=0;ci<hdrs.length;ci++){\n'
            '            var ca2=XLSX.utils.encode_cell({r:ri+1,c:ci});\n'
            '            if(ws[ca2])ws[ca2].s={font:{color:ci===0?{rgb:ip?"1D4ED8":"B45309"}:{rgb:"111111"},sz:10},fill:ip?pFill:aFill,border:bdr};\n'
            '          }\n'
            '        }\n'
            '        XLSX.utils.book_append_sheet(wb,ws,"CDG");\n'
            '        var fromV=(document.getElementById("cdg-from")||{}).value||"";\n'
            '        var toV=(document.getElementById("cdg-to")||{}).value||"";\n'
            '        var fname="Export "+(_an||"ORC")+" _ CDG _ "+fromV+" _ "+toV+".xlsx";\n'
            '        XLSX.writeFile(wb,fname);\n'
            '      }\n'
            '      if(window.XLSXStyle){doExport(window.XLSXStyle);}\n'
            '      else{\n'
            '        var sc=document.createElement("script");\n'
            '        sc.src="https://cdn.jsdelivr.net/npm/xlsx-js-style@1.2.0/dist/xlsx-js-style.min.js";\n'
            '        sc.onload=function(){doExport(window.XLSXStyle);};\n'
            '        sc.onerror=function(){alert("Impossible de charger la bibliothèque d\'export. Vérifiez votre connexion Internet.");};\n'
